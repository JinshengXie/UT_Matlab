% get_obj_func_ulr根据输入的参数，响应变量和解释变量，输出参数估计的目标函数值
...
%                  + ... + beta_p * x_p + epsilon的参数估计目标函数值
% ———————————————————————————————————————————
function f = get_obj_func_ULR(list_para,list_y_i,matrix_x_ij)
    % ---参数解析与验证---
    ...
    % ---运算部分---
    list_epsilon = get_resi_ULR(list_para,list_y_i,matrix_x_ij);
    % 得到残差列向量
    f = sum(list_epsilon.^2);
    % 求目标函数值，即残差的平方和
end